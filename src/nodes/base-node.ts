...
import { globalBlobStore } from "../engine/blob-store.js";
import type { EngineContext } from "../engine/engine-types.js";
import type { DagNodeStatus } from "../engine/dag-graph.js";
import type { FrozenSnapshot } from "../engine/snapshot-store.js";
...
export abstract class BaseNode<C extends EngineContext = any> {
  ...
  async execute(ctx: C): Promise<void> {
    // interceptor/bus 仅 RunContext 场景持有，此处通过 any 安全访问
    const interceptor = (ctx as any).interceptor as
          ...
          switch (decision.action) {
            ...
            case "edit":
              ...
              }
              {
                ...
                if (!validation.ok) {
                  (ctx as any).bus?.dispatch("node.validation_error", {
                    nodeId: this.id,
